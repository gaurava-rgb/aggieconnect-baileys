/**
 * Aggie Connect — System Monitor
 * TUI-style health dashboard for bot ops.
 * Usage: node monitor.js (runs on port 3001)
 */
...
async function fetchAll() {
    const [
        ...
        pm2List(),
        sb.from('baileys_requests').select('request_type, request_status, created_at'),
        sb.from('baileys_matches').select('id', { count: 'exact', head: true }),
        sb.from('monitored_groups').select('group_name, group_id, active').order('group_name'),
        sb.from('baileys_message_log')
...
async function buildReport() {
    ...
    // ── header ──
    ln(`  AGGIE CONNECT — SYSTEM MONITOR (BAILEYS)                    ${nowStr} CST`);
    hr('═');
    ...
    if (!pm2) {
        ...
        ln(`  ${'NAME'.padEnd(14)} ${'STATUS'.padEnd(10)} ${'UPTIME'.padEnd(10)} ${'RESTARTS'.padEnd(10)} ${'MEM'.padEnd(8)} CPU`);
        hr('·');
        for (const p of pm2) {
            const env = p.pm2_env || {};
            ...
            ln(`  ${icon} ${col(p.name, 13)} ${col(status, 10)} ${col(uptime, 10)} ${col(restarts, 10)} ${col(mem, 8)} ${cpu}`);
        }
    }

    // ── whatsapp state (inferred) ──
    section('WHATSAPP CONNECTION (inferred)');
    const botProc = (pm2 || []).find(p => p.name === 'aggie-baileys-bot');
    ...
    if (botStatus !== 'online') {
        ...
        if (secsSinceMsg > 600) {
            ...
            ln(`    (watchdog checks every 5m and will force-restart if state ≠ CONNECTED)`);
        }
    }
    ln(`  Restarts: ${botRestarts}   Watchdog: polls every 5m via client.getState()`);
    ...
    const maxP = Math.max(trueCount, total20 - trueCount, 1);
    ...
    const raw = await tailLog('/root/.pm2/logs/aggie-baileys-bot-error.log', 15);
    const errLines = raw.trim().split('\n')
        .map(l => l.replace(/^0\|aggie-bo \| /, '').trim())
        .filter(l => l.length > 0)
        .slice(-10);
...
<title>Aggie Connect — Baileys Monitor</title>
...
<p class="footer">
  ...
  <a href="http://localhost:3002">→ ride dashboard</a>
</p>
</body>
</html>`);
    } catch (err) {
